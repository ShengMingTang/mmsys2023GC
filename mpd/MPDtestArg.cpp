...
int main(int argc, char** argv)
{
    ...
    if(argc < 3) {
        spdlog::warn("not enough argument json provided, use default");
    }
    else {
        std::cout << "Param Json file :" << argv[2] << std::endl;
        std::ifstream f(argv[2]);
        if(f.fail()) {
            spdlog::error("Param Json file does not exist. Back to Default");
        }
        else {
            param = json::parse(f);
        }
    }
    std::cout << param.dump() << std::endl;
    ...
    myTransportCtlConfig->minWnd = param["minWnd"];
    myTransportCtlConfig->maxWnd = param["maxWnd"];
    myTransportCtlConfig->slowStartThreshold = param["slowStartThreshold"];
    myTransportCtlConfig->ccType = param["ccType"];
    myTransportCtlConfig->multiPathType = param["multiPathType"];
