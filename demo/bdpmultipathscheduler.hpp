...
class BDPNoLossMultiPathScheduler : public MultiPathSchedulerAlgo
{
public:
    MultiPathSchedulerType SchedulerType() override
    {
        return MultiPathSchedulerType::MULTI_PATH_SCHEDULE_BDP_NOLOSS;
    }

    explicit BDPNoLossMultiPathScheduler(const fw::ID& taskid,
    ...
    ~BDPNoLossMultiPathScheduler() override
    {
        SPDLOG_TRACE("");
    }
    ...
    void SortSession(std::multimap<Duration, fw::shared_ptr<SessionStreamController>>& sortmmap) override
    {
        ...
        {
            auto score = Duration::FromSeconds((int64_t)((double)sessionItor.second->GetCWND() * sessionItor.second->GetRtt().ToSeconds() * (1.0 - sessionItor.second->GetLossRate()) * (-1)));
            // auto score = Duration::FromSeconds((int64_t)((double)sessionItor.second->GetCWND() * sessionItor.second->GetRtt().ToSeconds()));
...
class BDPWithLossMultiPathScheduler : public MultiPathSchedulerAlgo
{
public:
    MultiPathSchedulerType SchedulerType() override
    {
        return MultiPathSchedulerType::MULTI_PATH_SCHEDULE_BDP_WITHLOSS;
    }

    explicit BDPWithLossMultiPathScheduler(const fw::ID& taskid,
            std::map<fw::ID, fw::shared_ptr<SessionStreamController>>& dlsessionmap,
            std::set<DataNumber>& downloadQueue, std::set<int32_t>& lostPiecesQueue)
            : MultiPathSchedulerAlgo(taskid, dlsessionmap, downloadQueue, lostPiecesQueue)
    {
    ...
    ~BDPWithLossMultiPathScheduler() override
    {
        SPDLOG_TRACE("");
    }
