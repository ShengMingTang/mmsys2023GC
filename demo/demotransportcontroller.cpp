        ...
        std::weak_ptr<MPDTransCtlHandler> transCtlHandler)
{
    ...
    switch (m_transCtlConfig->multiPathType)
    {
    case MultiPathSchedulerType::MULTI_PATH_SCHEDULE_RR:
        m_multipathscheduler.reset(
            new RRMultiPathScheduler(m_tansDlTkInfo.m_rid, m_sessStreamCtlMap, m_downloadPieces, m_lostPiecesl));
        break;
    case MultiPathSchedulerType::MULTI_PATH_SCHEDULE_BDP_NOLOSS:
        m_multipathscheduler.reset(
            new BDPNoLossMultiPathScheduler(m_tansDlTkInfo.m_rid, m_sessStreamCtlMap, m_downloadPieces, m_lostPiecesl));
        break;
    case MultiPathSchedulerType::MULTI_PATH_SCHEDULE_BDP_WITHLOSS:
        m_multipathscheduler.reset(
            new BDPNoLossMultiPathScheduler(m_tansDlTkInfo.m_rid, m_sessStreamCtlMap, m_downloadPieces, m_lostPiecesl));
        break;
    default:
        m_multipathscheduler.reset(nullptr);
        break;
    }
    if(m_multipathscheduler == nullptr) {
        return false;
    }
